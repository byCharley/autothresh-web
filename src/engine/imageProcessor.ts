...
// ─── Continuous edge screen ───────────────────────────────────────────────────
//
// Returns a Float32Array [0,1] per pixel where 0 = "dot centre" (most ink) and
// 1 = "furthest from dot" (no ink).  Used for bg-removal edge softness so that
// coverage (from the blurred bgMask) controls DOT SIZE, not just a binary cut:
//   • coverage = 0  → no dots  (pixel at background edge)
//   • coverage = 0.5 → medium dots
//   • coverage = 1  → solid    (deep interior, all pixels kept)
//
// For grain/noise the existing continuous noise values already work as a screen.
// For halftone patterns the binary buildPatternValues won't fade, so we compute
// a normalised distance-from-centre that scales smoothly with coverage.
export function buildEdgePatternScreen(
  w: number, h: number,
  pattern: PatternType,
  scale: number,
  angle: number,
): F32 | null {
  if (pattern === 'none' || pattern === 'dither-floyd') return null;

  // Grain / noise — already continuous, reuse directly
  if (pattern === 'noise' || pattern === 'grain' || pattern === 'grain-soft')
    return noiseValues(w, h, scale, 1);
  if (pattern === 'noise-coarse' || pattern === 'grain-coarse')
    return noiseCoarseValues(w, h, scale, 1);
  if (pattern === 'bayer-2') return bayerValues(w, h, scale, 2);
  if (pattern === 'bayer-4') return bayerValues(w, h, scale, 4);
  if (pattern === 'bayer-8') return bayerValues(w, h, scale, 8);
  if (pattern === 'reticulation') return reticulationValues(w, h, scale, 50, 1);
  if (pattern === 'grain-micro') return noiseValues(w, h, 1, 1);

  // Halftone patterns — compute continuous distance-from-dot-centre so that
  // edgeScreenVal < coverage produces dots that grow with coverage.
  const vals = new Float32Array(w * h) as F32;
  const ar   = (angle * Math.PI) / 180;
  const cosA = Math.cos(ar), sinA = Math.sin(ar);
  const freq = Math.max(2, scale);
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const rx =  x * cosA + y * sinA;
      const ry = -x * sinA + y * cosA;
      const cx = ((rx % freq) + freq) % freq - half;
      const cy = ((ry % freq) + freq) % freq - half;
      let d: number;
      switch (pattern) {
        case 'halftone-round':
          d = Math.sqrt(cx * cx + cy * cy) / half; break;
        case 'halftone-diamond':
          d = (Math.abs(cx) + Math.abs(cy)) / half; break;
        case 'halftone-ellipse':
          d = Math.sqrt((cx / 1.5) ** 2 + (cy / 0.6) ** 2) / half; break;
        case 'halftone-square':
          d = Math.max(Math.abs(cx), Math.abs(cy)) / half; break;
        case 'halftone-cross':
          // distance from nearest cross arm
          d = Math.min(Math.abs(cx), Math.abs(cy)) / half; break;
        case 'halftone-line':
        case 'halftone-line-am':
        case 'halftone-line-fm':
          d = Math.abs(cx) / half; break;
        case 'halftone-crosshatch':
          d = Math.min(Math.abs(cx), Math.abs(cy)) / half; break;
        case 'halftone-wave': {
          const wave = Math.sin((ry / freq) * 2 * Math.PI);
          d = Math.abs(cx + half * 0.5 * wave) / half; break;
        }
        default:
          d = Math.sqrt(cx * cx + cy * cy) / half; break;
      }
      vals[y * w + x] = Math.min(1, d);
    }
  }
  return vals;
}
...
export function processImage(
  ...
  // ── 2b. One shared pattern for all global-pattern layers (continuous film overlay)
...
export function scaleImageData(imageData: ImageData, maxDim: number): ImageData {
  ...
  (src.getContext('2d', { colorSpace: 'srgb' }) as CanvasRenderingContext2D).putImageData(imageData, 0, 0);
  const dst = document.createElement('canvas');
  dst.width = sw; dst.height = sh;
  const dstCtx = dst.getContext('2d', { colorSpace: 'srgb' }) as CanvasRenderingContext2D;
...
export function scaleImageDataExact(imageData: ImageData, targetW: number, targetH: number): ImageData {
  ...
  const ctx = dst.getContext('2d', { colorSpace: 'srgb' }) as CanvasRenderingContext2D;
